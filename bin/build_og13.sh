#!/bin/bash
#
#  build_og13.sh: script to configure and build gcc for amdgcn
#
#  This runs a long time so run this script with nohup like this:
#    nohup build_og13.sh
#  or 
#   ./build_og13.sh 2>&1 | tee build.out
#
...
if [ -d $gccmaindir ] ; then 
   ...
   cd $gccmaindir
   git checkout devel/omp/gcc-13
   git pull
fi

binutilsmaindir=$OGDIR/binutils-gdb
if [ -d $binutilsmaindir ] ; then 
   # Get updates
   cd $binutilsmaindir
   git pull
   git status
   ...
   echo cd $OGDIR
   cd $OGDIR
   echo git clone https://sourceware.org/git/binutils-gdb
   git clone https://sourceware.org/git/binutils-gdb
   cd $binutilsmaindir
   # FIXME: is this the correct branch to build?
   git checkout master
   git pull
fi
...
if [ $? != 0 ] ; then 
   echo "ERROR: mkdir build-binutils"
   exit 1
fi
echo cd build-binutils
cd build-binutils
../binutils-gdb/configure --prefix=$OG_INSTALL_DIR -v 
if [ $? != 0 ] ; then 
   echo "ERROR configure binutils failed"
   exit 1 
fi

echo " ============================ OG13BSTEP: binutils make ================="
make -j$OG_BUILD_PROCS
if [ $? != 0 ] ; then 
   echo "ERROR  make binutils failed"
   exit 1
fi
echo " ============================ OG13BSTEP: binutils install ================="
make install 
if [ $? != 0 ] ; then 
   echo "ERROR: make install for binutils failed"
   exit 1
fi
echo " ============================ OG13BSTEP: Configure device ================="
cd $OGDIR
...
echo " ============================ OG13BSTEP: device make ================="
make -j$OG_BUILD_PROCS
if [ $? != 0 ] ; then 
   echo "ERROR  make amdgcn compiler failed"
   exit 1
fi
echo " ============================ OG13BSTEP: device make install ================="
make install 
if [ $? != 0 ] ; then 
...
echo cd build-host
cd build-host
echo "../gcc/configure --prefix=$OG_INSTALL_DIR -v --with-pkgversion=\"AMD-OG13 Sourcery CodeBench (AMD GPU) : $GIT_ID\" --build=x86_64-pc-linux-gnu --host=x86_64-pc-linux-gnu --target=x86_64-pc-linux-gnu --enable-threads --disable-libmudflap --disable-libstdcxx-pch --disable-multilib --with-gnu-as --with-gnu-ld --enable-languages=c,c++,fortran --enable-shared --enable-lto --disable-nls --enable-libgomp --disable-libitm --enable-libatomic --disable-libssp --disable-libcc1 --enable-offload-targets=amdgcn-amdhsa=$installdir/amdgcn-amdhsa"
../gcc/configure --prefix=$OG_INSTALL_DIR -v --with-pkgversion="AMD-OG13 Sourcery CodeBench (AMD GPU) : $GIT_ID" --build=x86_64-pc-linux-gnu --host=x86_64-pc-linux-gnu --target=x86_64-pc-linux-gnu --enable-threads --disable-libmudflap --disable-libstdcxx-pch \
   ...
   --enable-offload-targets=amdgcn-amdhsa=$OG_INSTALL_DIR/amdgcn-amdhsa 2>&1 | tee ../hostconfig.stdout
if [ $? != 0 ] ; then 
   echo "ERROR configure host compiler failed"
   exit 1 
fi

echo " ============================ OG13BSTEP: host make ================="
make -j$OG_BUILD_PROCS
if [ $? != 0 ] ; then 
   echo "ERROR  make host compiler failed"
   exit 1
fi
echo " ============================ OG13BSTEP: host install ================="
make install 
if [ $? != 0 ] ; then 
   echo "ERROR: make install failed"
   exit 1
